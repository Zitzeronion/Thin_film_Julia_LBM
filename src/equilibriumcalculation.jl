...
function calc_equilibrium_distribution(mom::moments1d_64, gravity=0)
    ...
    v = 1.0
    vsquare = v^2
    equi_dist = distribution64_1d(zeros(len), zeros(len), zeros(len))
...
function calc_equilibrium_distribution(mom::moments1d_32, gravity=0)
    len = length(mom.height)
    v = 1.0
    vsquare = v^2
    equi_dist = distribution32_1d(zeros(len), zeros(len), zeros(len))
